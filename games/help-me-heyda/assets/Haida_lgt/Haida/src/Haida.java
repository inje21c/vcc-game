/*
1.스토리 모드 토템에 붙은 깃털 하나 습득시 깃털이 계속 올라갑니다( sp포함.. )
2.랭킹에서 스토리와 서바이벌 조금 왔다갔다 하다가 메뉴로 나오려 하면.. 나올수가 없습니다.
*/
	...
	public void keyPressed(int nKey) {
		...
		case SCENE_RANK_REGIST:
			if(nKey == KEY_STAR)
			{
				bKeyEnable = false;
				if(rs == null)
					rs = new RankSoket(this);
				rs.writeUserInfo();
				rs.writeScore(nStoryScore[4], nSurvivalScore[4]);
...
*/					}
			...
			if( nMotion==Canvas.FIRE ) {
				if( (realStage[10][0]==0) && (realStage[11][0]==0) ) {	//스테이지가 클리어 되었다면..
					...
					if(saveRecord(storyGameScore, 1))
						nCurrentScene = SCENE_RANK_REGIST;
					else
						nCurrentScene = SCENE_MENU;
				}
		...
		case SCENE_GAME_SURVIVAL:
			...
			if( nMotion==Canvas.FIRE ) {
				if( gameClearFlag == 2 ) {	//fail
					...
					skillPointNum = 0;
					menuNum = 0;
					if(saveRecord(survivalGameScore,2))
						nCurrentScene = SCENE_RANK_REGIST;
					else
						nCurrentScene = SCENE_MENU;
		...
		case SCENE_GAME_SCORE:
			if( nMotion==Canvas.FIRE ) {
				storyGameScore += ((gameTime*5)-(mistakeFlag*5));
		...
		case SCENE_SCORE:
			if(nMotion == RIGHT && nRankMode == 1)		nRankMode = 2;
			else if(nMotion == LEFT && nRankMode == 2) 	nRankMode = 1;
			else if(nMotion == FIRE)
			{
				nCurrentScene = nReturnScene;
				nReturnScene = -1;
			}
...
//		bufferedGraphics.setColor(0xffe300);
	...
	private void drawRank()
	{
		...
		bufferedGraphics.setColor(0xFFC0FF);
		if(nRankMode == 1)
			bufferedGraphics.drawString("◀ 스토리 랭킹 ▶", DISPLAY_WIDTH/2, 5, Graphics.TOP|Graphics.HCENTER);
		else if(nRankMode == 2)
			bufferedGraphics.drawString("◀ 서바이벌 랭킹 ▶", DISPLAY_WIDTH/2, 5, Graphics.TOP|Graphics.HCENTER);

		bufferedGraphics.setColor(0xffffff);
		for(int i=0; i<4; i++) {
			if( ( nRankMode==1?nStoryScore[i]:nSurvivalScore[i] ) == -1 )	break;
			...
			if(nRankMode == 1)
				...
				bufferedGraphics.drawString(String.valueOf(nSurvivalScore[i]), DISPLAY_WIDTH-15, (16*i)+27, Graphics.TOP|Graphics.RIGHT);
		}

//		bufferedGraphics.setColor(0);
		bufferedGraphics.drawLine(0, 88, 120, 88);
//		bufferedGraphics.setColor(0xffffff);
		bufferedGraphics.drawString("나의 랭킹: " + (nRankMode==1?nMyRank[0]:nMyRank[1]), 10, 91, TOPLEFT);
		bufferedGraphics.drawString("나의 점수: " + (nRankMode==1?nStoryScore[4]:nSurvivalScore[4]), 10, 105, TOPLEFT);
	}
	...
	private void drawHelp()
	{
		...
		bufferedGraphics.drawString( "[ ◀ ] 블럭밀기", 10, 55, TOPLEFT );
		bufferedGraphics.drawString( "[ ▶ ] 기술사용" , 10, 75, TOPLEFT );
//		bufferedGraphics.drawString( "[취소] 일시정지",10, 89, TOPLEFT );
	...
	private void drawStageEnding()
	{
		...
		if(nSin == 0)
		{
			bufferedGraphics.drawString("토템의 힘이", 10, 25, TOPLEFT);
			bufferedGraphics.drawString("모여서 강력한", 10, 40, TOPLEFT);
			bufferedGraphics.drawString("벽을 만들었다.", 10, 55, TOPLEFT);
			bufferedGraphics.drawString("불도저는 벽을", 10, 70, TOPLEFT);
			bufferedGraphics.drawString("없애지 못해", 10, 85, TOPLEFT);
			bufferedGraphics.drawString("포기하고 마을을", 10, 100, TOPLEFT);
		}else if(nSin == 1)
		{
			bufferedGraphics.drawString("떠났고, 헤이다는", 10, 25, TOPLEFT);
			bufferedGraphics.drawString("인디언 마을을", 10, 45, TOPLEFT);
			bufferedGraphics.drawString("구할수 있었다.", 10, 65, TOPLEFT);
...
*/
	public boolean saveRecord(int score, int mode) {
		if( score <= (mode==1?nStoryScore[4]:nSurvivalScore[4]) )
			return false;
...
class RankSoket extends Thread
{
	...
	private byte[] rb = new byte[80];

	private int cnt = 0, dataLen = 0;
	...
	public void run()
	{
		...
		try{
			...
			t = 0;
			header[0] = (rb[3] << 24) | (rb[2] << 16) | (rb[1] << 8) | rb[0];
			header[1] = (rb[7] << 24) | (rb[6] << 16) | (rb[5] << 8) | rb[4];
			...
			System.out.println("Receive Data Exception == " + ioe.getMessage());
		}
	}
	...
	void readData()
	{
		for(int cnt=0; cnt<4; cnt++)
			canvas.nStoryScore[cnt] = ((rb[cnt*8]&0x0FF)<<24) | ((rb[cnt*8+1]&0x0FF)<<16) | ((rb[cnt*8+2]&0x0FF)<<8) | (rb[cnt*8+3]&0x0FF);
		for(int cnt=0; cnt<4; cnt++)
			canvas.nSurvivalScore[cnt] = ((rb[cnt*8+32]&0x0FF)<<24) | ((rb[cnt*8+33]&0x0FF)<<16) | ((rb[cnt*8+34]&0x0FF)<<8) | (rb[cnt*8+35]&0x0FF);
		canvas.nStoryScore[4] = ((rb[64]&0x0FF)<<24) | ((rb[65]&0x0FF)<<16) | ((rb[66]&0x0FF)<<8) | (rb[67]&0x0FF);
		canvas.nMyRank[0] = ((rb[68]&0x0FF)<<24) | ((rb[69]&0x0FF)<<16) | ((rb[70]&0x0FF)<<8) | (rb[71]&0x0FF);
		canvas.nSurvivalScore[4] = ((rb[72]&0x0FF)<<24) | ((rb[73]&0x0FF)<<16) | ((rb[74]&0x0FF)<<8) | (rb[75]&0x0FF);
		canvas.nMyRank[1] = ((rb[76]&0x0FF)<<24) | ((rb[77]&0x0FF)<<16) | ((rb[78]&0x0FF)<<8) | (rb[79]&0x0FF);
		canvas.setScene();
		rb = null;
		System.gc();
	}
	...
	void writeData(byte[] sb)
	{
		...
		try{
			...
			System.out.println("Send Data Exception == " + ioe.getMessage());
		}
	}
}